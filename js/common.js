...
window.onload = function() {
	...
	/*	var timer;
		$("#navigation_main_x_mainsuboffer_x_models").mouseover(function() {
			if(timer) {
				clearTimeout(timer);
			}
			$("html").addClass("noscroll");
		...
		$("#navigation_main_x_mainsuboffer_x_models").mouseout(function() {
	...
	*/
	...
	function fnBurgerMenu() {
		...
		/*$('body').unbind('mousewheel').bind('mousewheel', function(event, delta) {
			var selftop2 = $(this).scrollTop();
			var dir = delta > 0 ? 'Up' : 'Down';
			if(dir == 'Up' && (selftop2 <= 100)) {
				$("#ui").removeClass("smallView");
			} else {
